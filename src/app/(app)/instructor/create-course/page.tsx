...
export default function InstructorCreateCoursePage() {
  ...
  async function onBasicSubmit(values: Pick<CourseFormValues, 'courseTitle' | 'courseDescription' | 'courseCategory' | 'courseThumbnail'>) {
    setIsSubmittingBasic(true);
    if (!user || !user.name) {
        toast({ title: "Error de Autenticación", description: "No se pudo identificar al instructor.", variant: "destructive" });
      ...
      <div className="container mx-auto py-8 px-4 md:px-0">
        <div className="flex items-center mb-6">
          ...
          <h1 className="text-3xl font-bold text-primary font-headline">
            {currentCourse ? `Editando Curso: ${currentCourse.title}` : "Crear Nuevo Curso"}
          </h1>
        </div>
        ...
        <Form {...form}>
          {!currentCourse ? (
            <Card className="max-w-3xl mx-auto shadow-lg">
              ...
              <CardContent>
                <form onSubmit={form.handleSubmit(data => onBasicSubmit(data as Pick<CourseFormValues, 'courseTitle' | 'courseDescription' | 'courseCategory' | 'courseThumbnail'>))} className="space-y-6">
                  ...
                  <FormField control={form.control} name="courseDescription" render={({ field }) => (<FormItem><FormLabel>Descripción Breve</FormLabel><FormControl><Textarea placeholder="Describe de qué trata tu curso..." {...field} /></FormControl><FormMessage /></FormItem>)} />
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="w-full">
                      <label htmlFor="dropzone-file-instructor" className={cn("flex flex-col items-center justify-center w-full h-48 border-2 border-border border-dashed rounded-lg cursor-pointer relative overflow-hidden", thumbnailPreview ? "border-primary bg-card" : "bg-muted/50 hover:bg-muted/70")}>
                        ...
                        <Input id="dropzone-file-instructor" type="file" className="hidden" accept="image/png, image/jpeg" onChange={handleThumbnailChange} />
                      </label>
                  ...
                  <Button type="submit" className="w-full sm:w-auto" disabled={isSubmittingBasic}>
                    ...
                    Guardar Información Básica y Añadir Módulos
                  </Button>
                </form>
              </CardContent>
            </Card>
            ...
            <Card className="max-w-3xl mx-auto shadow-lg">
              <form onSubmit={form.handleSubmit(onModulesSubmit)}>
                <CardHeader>
                  ...
                  <CardDescription>Organiza el contenido de tu curso en módulos y lecciones.</CardDescription>
                </CardHeader>
                          ...
                          <div className="flex-1 flex items-center justify-between pr-2">
                             <FormField
                                ...
                                render={({ field }) => (
                                  <Input
                                    {...field}
                                    placeholder={`Título del Módulo ${moduleIndex + 1}`}
                                    className="text-lg font-semibold flex-grow border-0 shadow-none focus-visible:ring-0 p-0 h-auto"
                                    onClick={(e) => e.stopPropagation()} 
                                  />
                                )}
                              />
                            <Button variant="ghost" size="icon" onClick={(e) => {e.stopPropagation(); removeModule(moduleIndex);}} className="text-destructive hover:text-destructive-foreground hover:bg-destructive/90 ml-2">
                           ...
                           {(form.getValues(`modules.${moduleIndex}.lessons`) || []).map((lessonItem, lessonIndex) => (
                            <Card key={lessonItem.id} className="bg-card p-4 shadow-sm">
                              <div className="flex justify-between items-center mb-2">
                                ...
                                />
                                <Button variant="ghost" size="icon" onClick={() => removeLessonFromModule(moduleIndex, lessonIndex)} className="text-destructive hover:text-destructive-foreground hover:bg-destructive/90">
                                  <Trash2 className="h-4 w-4" />
                                </Button>
                              </div>
                               <FormField
